...
def MSAEA(g, bench):
    ...
    for i in range(POPULATION_SIZE):
        Strategy = np.random.choice(Archives, p=[0.25, 0.25, 0.25, 0.25])
        Strategy(i, g, bench)
        g += 1
    bestSelection()
    return g
...
def RunMSAEA():
    ...
    np.savetxt('./MSAEA_0.25_Data/F{}_{}D.csv'.format(Fun_num, DIMENSION_NUM), All_Trial_Best, delimiter=",")
...
if __name__ == "__main__":
    if os.path.exists('./MSAEA_0.25_Data') == False:
        os.makedirs('./MSAEA_0.25_Data')
    main_10()
    # main_50()